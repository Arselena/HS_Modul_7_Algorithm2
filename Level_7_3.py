...
class BST: # BinarySearchTree
    ...
    def WideAllNodes(self):
        return BST.tree_traversal(self.Root, [self.Root], [self.Root])

    def DeepAllNodes(self, order):
        def DeepAllNodes_recurs(node):
            if node is None:
                return []
            AllNodes = []
            if order == 2:  # прямой (префиксный, pre-ordered): вершина – левое поддерево – правое поддерево
                AllNodes.append(node)
            AllNodes.extend(DeepAllNodes_recurs(node.LeftChild))
            if order == 0:  # обратный (инфиксный, in-ordered): левое поддерево – вершина – правое поддерево
                AllNodes.append(node)
            AllNodes.extend(DeepAllNodes_recurs(node.RightChild))
            if order == 1:  # концевой (постфиксный, post-ordered): левое поддерево – правое поддерево – вершина
                AllNodes.append(node)
            return AllNodes
        return DeepAllNodes_recurs(self.Root)