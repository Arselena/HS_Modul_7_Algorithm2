...
class SimpleGraph:
    ...
    def WeakVertices(self):
        def unique_pairs(n): # генератор уникальных пар
            for i in range(n):
        ...
        for i in range(self.max_vertex): 
            ...
            Child_for_i = []
            for j in range(self.max_vertex):
                if i != j and self.IsEdge(i, j) == True:
                    Child_for_i.append(j)
            
            if Child_for_i == []:
                continue
            
            for i_ch, j_ch in unique_pairs(len(Child_for_i)):
                if i_ch != j_ch and self.IsEdge(Child_for_i[i_ch], Child_for_i[j_ch]) == True:
                    Weak_Vertices.remove(self.vertex[i])
                    break
